...
type OrderWithOrderProductsDocument = OrderDocument & {
  orderProduct?: OrderProductDocument[]
}
  ...
  static toDomain(entity: OrderWithOrderProductsDocument): Order {
    const model = new Order({
      ...
      orderProduct: !!entity.orderProduct
        ? entity.orderProduct.map((order) =>
            MongooseOrderProductMapper.toDomain(order)
          )
        : [],
