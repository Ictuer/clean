...
export class MongooseOrderProductRepository implements OrderProductRepository {
  constructor(
    @InjectModel(OrderProductMongoose.name)
    private readonly productOrderModel: Model<OrderProductMongoose>
  ) {}
  ...
  async createMany(orderProducts: OrderProduct[]): Promise<OrderProduct[]> {
    ...
    const data = orderProducts.map((product) =>
      MongooseOrderProductMapper.toMongoose(product)
    )

    const createdOrderProductProducts =
      await this.productOrderModel.create(data)
