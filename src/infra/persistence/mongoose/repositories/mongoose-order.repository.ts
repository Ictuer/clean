...
export class MongooseOrderRepository implements OrderRepository {
  constructor(
    @InjectModel(OrderMongoose.name)
    private readonly orderModel: Model<OrderMongoose>,
    private orderProductRepository: OrderProductRepository,
    private userRepository: UserRepository
  ) {}

  async findMany(): Promise<Order[]> {
    const findQuery = await this.orderModel.find().populate(['orderProduct'])
